view: rs_logrecno_bg_map {
    ...
    derived_table: {
      ...
      FROM
        looker_blocks.geo2015 as geo
      LEFT JOIN looker_blocks.block_group_attribs as bg on (SUBSTRING(geo.GEOID, 8, 12) = bg.geoid AND geo.SUMLEVEL = '150')
      LEFT JOIN looker_blocks.block_group_attribs as tr on (SUBSTRING(geo.GEOID, 8, 11) = SUBSTRING(tr.geoid, 1, 11) AND geo.SUMLEVEL = '140')
      WHERE
        sumlevel in ('140','150')
